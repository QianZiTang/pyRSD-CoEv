...
def readVcf(vcffile):
	...
	chrfile=open(os.path.join(outpath,'chrom_sizes.txt'),'w')
	indexfile=open(os.path.join(outpath,'index.txt'),'w')
	...
	for i in range(0,len(ind_names)):
		indexfile.write('%s\t%s\n'%(i,ind_names[i]))
	indexfile.close()
