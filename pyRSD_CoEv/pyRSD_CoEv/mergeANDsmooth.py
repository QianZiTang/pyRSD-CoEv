...
def main():
	...
	upperpath=os.path.abspath(os.path.join(pwd_dir, ".."))
	out_dir=upperpath+'/03mergeAndsmooth'
	...
	chr_name=args.chrom[0]


	mergefile='%s/%s_merged.txt'%(out_dir,chr_name)
	
	all_combin,needpvalue,mergedinfo=mergeRsdWindows(args.input[0].name,mergefile)
	print('***finish merge window***')

	print('***start shuffle file***')
	shufflefile='%s/%s_merged_shuffled.txt'%(out_dir,chr_name)

	smooth(all_combin,needpvalue,mergedinfo,args.shuffle,shufflefile)
	print('***finish shuffle file***')

	print('***strat correct P value***')
	correctfile='%s/%s_merged_shuffled_fdr.txt'%(out_dir,chr_name)
