...
def parse_arguments():
	parser = argparse.ArgumentParser(
		...
		add_help=False,
		description=('Create an m x n matrix of evolutionary rate was obtained, where m represents the number of strains and n represents the number of transcripts.'))
...
def getfdr(correlationfile,out):
	R = subprocess.getoutput('which Rscript')
	path=os.path.dirname(os.path.abspath(__file__))
	cmd='Rscript %s %s %s'%(os.path.join(path,'fdr.R'),1,correlationfile,out)
	os.system(cmd)
